...
def download(get_url):
    @wraps(get_url)
    def wrapper(*args, timeout=100, requests_interval=10, max_requests_number=100, break_reasons=None):
        """
...
@parse_html
@download
def resume_search(request, num_pages):
    print(request)		
    """
    :param str area_id: area identifier from https://api.hh.ru/areas
    :param str specialization_id: specialization identifier from https://api.hh.ru/specializations
    ...
    :return: str
    """
    return request + f"&page={num_pages}"



def search_resume_ids(request):
    print(request)
    print("********************************************")
    """
    :param area_id: area identifier from https://api.hh.ru/areas
    ...
    for num_page in range(num_pages - 1):
        page = resume_search(request, num_page)
        ids.extend(parse_resume_hashes(page))
        print(ids)
